...
def showSetup(dataManager):
    ...
    discordHook = config.get("discord", "webhook", fallback="discord webhook url")
    ...
    app.addLabel("sep4", " ", 12, 0)
    entryDiscordHook = "Notification via Discord : "
    app.addLabel(entryDiscordHook, entryDiscordHook, 13, 0)
    app.addValidationEntry(entryDiscordHook, 13, 1)
    app.setEntry(entryDiscordHook, discordHook)
    app.setEntryWidth(entryDiscordHook, 80)
    ...
    def validSetup():
        ...
        discordHook = app.getEntry(entryDiscordHook)
        app.setEntryValid(entryDiscordHook)
        if not len(discordHook) > 25:
            valid = False
            app.setEntryInvalid(entryDiscordHook)
...
def showApplication(dataManager, socketManager, tradingManager):
    ...
    discordNotifyCheckBox = "Notification via webhook Discord"
    ...
    app.addCheckBox(discordNotifyCheckBox)
    if dataManager.withDiscordNotify:
        app.setCheckBox(discordNotifyCheckBox)
    ...
    def updateTrade():
        try:
            ...
            withDiscordNotify = app.getCheckBox(discordNotifyCheckBox)
            if withDiscordNotify != dataManager.withDiscordNotify:
                updatePref = True
                dataManager.withDiscordNotify = withDiscordNotify
                if withDiscordNotify:
                    app.setCheckBox(discordNotifyCheckBox)
                    dataManager.withDiscordNotify = True
